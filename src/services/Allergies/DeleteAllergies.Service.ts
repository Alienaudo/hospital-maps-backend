...
export class DeleteAllergieService {
    ...
    public exec = async (
        ...
        try {
            ...
            const { allergieId } = request.params;

            if (!uid) throw Error("Auth Middleware not privided user's uid");

            const result: BatchPayload = await this.prisma.allergies
                .deleteMany({
            ...
            logger.error(`Error deleting user's allergie: ${error}`);
